              ...
              for d, j in enumerate(i['params']['switches']):
                  ...
                  if (d < outlets): 
                     ...
                     if (state == 'on'):
                        ...
                        print ('%s%s %s%s%s | refresh=true terminal=false bash="%s" param1=%s param2=%s param3=%s param4=%s color=%s' % (prefix, justify(outletname,46), CRED, state, CEND, sys.argv[0],  'switch', devid, 'on', d, color))
                        print ('%s%s %s%s%s | refresh=true alternate=true terminal=true bash="%s" param1=%s param2=%s param3=%s param4=%s color=%s' % (prefix, justify(outletname,46), CRED, state, CEND, sys.argv[0], 'switch', devid, 'on', d, color))
       except:             # Just one switch 
          state = i['params']['switch']
          if (state == 'on'):
             print ('%s%s %s%s%s | refresh=true terminal=false bash="%s" param1=%s param2=%s param3=%s color=%s' % (prefix, justify(name,46), CGREEN, state, CEND, sys.argv[0], 'switch', devid, 'off', color))
             print ('%s%s %s%s%s | refresh=true alternate=true terminal=true bash="%s" param1=%s param2=%s param3=%s color=%s' % (prefix, justify(name,46), CGREEN, state, CEND, sys.argv[0], 'switch', devid, 'off', color))
          else:
             print ('%s%s %s%s%s | refresh=true terminal=false bash="%s" param1=%s param2=%s param3=%s color=%s' % (prefix, justify(name,46), CRED, state, CEND, sys.argv[0],  'switch', devid, 'on', color))
             print ('%s%s %s%s%s | refresh=true alternate=true terminal=true bash="%s" param1=%s param2=%s param3=%s color=%s' % (prefix, justify(name,46), CRED, state, CEND, sys.argv[0], 'switch', devid, 'on', color))
